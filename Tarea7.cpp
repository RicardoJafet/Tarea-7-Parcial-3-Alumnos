/*Autor: Ricardo Jafet Granados Chable  fecha: 09/05/2022
...
void ordenarpromedio(struct alumno x[], float tipo);
...
void ordenarpromedio(struct alumno x[], float tipo){
	//ordenamiento del arreglo por el metodo de la burbuja 
	for(promedio=0, promedio>=5, promedio++){
		struct alumno aux;
		print("/nCalificaciones:", promedio);
	}
imprimir(x,0);
}
